    ...
    #define HOLD_FRACTION 0.01f    // last fraction of move for gentle hold
    #define MIN_HOLD_RATE_HZ 10       // hold phase step rate
    #define MIN_DT_TICKS 2        // minimum timer ticks (ARR = dt-1)
    ...
    static uint32_t TimerClockHz(void) {
        ...
        if ((RCC->CFGR & RCC_CFGR_PPRE1) != RCC_CFGR_PPRE1_DIV1) pclk1 *= 2;
        return pclk1;
    }
    ...
    void on_menu_move(void) {
        ...
        accel_rate   = fabsf(accel_u_s2 * ppu);
        float v_req   = fabsf(vmax_u_s * ppu);
        ...
        if (dt0 < MIN_DT_TICKS) dt0 = MIN_DT_TICKS;
   ...
   void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim) {
        ...
        uint32_t hold = (uint32_t)(total_pulses * HOLD_FRACTION + 0.5f);
        if (pulses_left <= (int32_t)hold) {
            dt_ticks = (uint32_t)(timerTickHz / MIN_HOLD_RATE_HZ + 0.5f);
        }
        
        if (dt_ticks < MIN_DT_TICKS) 
            dt_ticks = MIN_DT_TICKS;
    ...
    void switchEXTIHandler(uint16_t GPIO_Pin) {
        ...
        if(HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_0) == GPIO_PIN_RESET && !homing_reverse_started) {
            arrowDir = !arrowDir;
            HAL_GPIO_WritePin(GPIOC, GPIO_PIN_9, arrowDir ? GPIO_PIN_RESET : GPIO_PIN_SET);
    ...
    void home(void) {
        ...
        float homing_pps = fabsf(vmax_u_s * ppu);
        if (homing_pps < 1.0f) 
            homing_pps = 1.0f;
